...
const Post = ({ post }) => {
	...
	const {data:authUser} = useQuery({
		queryKey: ["authUser"],
	}); //queryfn ke jarut nhi vo pahle hi app me hai
	...
	const {mutate: deletePost, isPending} = useMutation({
		mutationFn: async () =>{
			try {
				...
				const data = await res.json(); // are ye backend se a rha jo ham logo ne diya tha messages and errors
				if(!res.ok){
		...
		onSuccess: () =>{
			...
			queryClient.invalidateQueries({queryKey: ["posts"]}); //after deleting post is invalidate in refech posts
		}
	})
    ...
    const {mutate:likePost,isPending:isliking} = useMutation({
		...
		onSuccess:(updatedLikes) =>{
			...
			//queryClient.invalidateQueries({queryKey:["posts"]}); not good har post like per pura dobara render nhi kerna
	...
	const handleLikePost = () => {
		if(isliking) return; //if you liked if it is in loading state so you can't click again till that loading
		likePost();
	};
			...
			<div className='flex gap-2 items-start p-4 border-b border-gray-700'>
				<div className='avatar'>
					<Link to={`/profile/${postOwner.username}`} className='w-8 rounded-full overflow-hidden'>
						<img src={postOwner.profileImg || "/avtar_place_holder.png"} />
					</Link>
				</div>
											...
											<div key={comment._id} className='flex gap-2 items-start'>
												<div className='avatar'>
													<div className='w-8 rounded-full'>
														<img
															src={comment.user.profileImg || "/avtar_place_holder.png"}
														/>
													</div>
												</div>
